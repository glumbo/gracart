...
class MakePlugin extends Command
{
    ...
    public function plugin($code = 'Other', $key = '', $download = 0)
    {
        ...
        $pluginKey = gc_word_format_class($key);
        $pluginCode = gc_word_format_class($code);
        if (!in_array($pluginCode, $arrcodePlugin)) {
            $pluginCode = 'Other';
        }
        $pluginUrlKey = gc_word_format_url($key);
        ...
        try {
            File::copyDirectory(base_path('vendor/grakan/core/src/'.$source), storage_path($tmp));
            File::copyDirectory(base_path('vendor/grakan/core/src/'.$sourcePublic), storage_path($tmpPublic));
        ...
        try {
            if ($download) {
                ...
                gc_zip(storage_path($this->tmpFolder."/".$sID), $path);
            } else {
