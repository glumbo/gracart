...
class Backup extends Command
{
    ...
    public function handle()
    {
        ...
        if (count(glob(storage_path() . "/backups/*.sql")) >= self::LIMIT) {
            echo json_encode(['error' => 1, 'msg' => gc_language_render('admin.backup.limit_backup')]);
            exit;
        }
        if ($path) {
        ...
        try {
            $databaseName = config('database.connections.'.GC_CONNECTION.'.database');
            $userName = config('database.connections.'.GC_CONNECTION.'.username');
            $password = config('database.connections.'.GC_CONNECTION.'.password');
            $host = config('database.connections.'.GC_CONNECTION.'.host');
            $pathMysqlBin = config('database.connections.'.GC_CONNECTION.'.path_mysql_bin');// C:\xampp\mysql\bin
            ...
            gc_report($exception->getMessage());
