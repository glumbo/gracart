...
if (request()->method() == 'POST' && request()->ajax()) {
    ...
    switch ($step) {
        ...
        try {
            ...
            $getEnv = str_replace('gc_your_domain', $domain, $getEnv);
            $getEnv = str_replace('gc_database_host', $database_host, $getEnv);
            $getEnv = str_replace('gc_database_port', $database_port, $getEnv);
            $getEnv = str_replace('gc_database_name', $database_name, $getEnv);
            $getEnv = str_replace('gc_database_user', $database_user, $getEnv);
            $getEnv = str_replace('DB_PREFIX=gc_', 'DB_PREFIX='.$database_prefix, $getEnv);
            $getEnv = str_replace('gc_database_password', $database_password, $getEnv);
            $getEnv = str_replace('gc_api_key', $api_key, $getEnv);
            $getEnv = str_replace('gc_admin', $admin_url, $getEnv);
            $getEnv = str_replace('Asia/ho_chi_minh', $timezone_default, $getEnv);

            if (request('only_cms')) {
                $getEnv = str_replace('GC_ECOMMERCE_MODE=1', 'GC_ECOMMERCE_MODE=0', $getEnv);
            }
    ...
    echo view('grakan-front::install', array(
