...
use Glumbo\Gracart\Library\ShoppingCart\Exceptions\CartAlreadyStoredException;
use Glumbo\Gracart\Library\ShoppingCart\Exceptions\UnknownModelException;
use Closure;
...
class Cart
{
     ...
     * @return \Glumbo\Gracart\Library\ShoppingCart\Cart
     */
     ...
     * @return \Glumbo\Gracart\Library\ShoppingCart\CartItem
     */
    ...
    public function content()
    {
        ...
        foreach ($content as $key => $item) {
            $product = \Glumbo\Gracart\Front\Models\ShopProduct::where('id', $item->id)
    ...
    public function total()
    {
        ...
        $total = $content->reduce(function ($total, CartItem $cartItem) {
            return $total + ($cartItem->qty * gc_tax_price($cartItem->price, $cartItem->tax));
        }, 0);
        return $total;
    }
    ...
    */
    public static function getListCart($instance = self::DEFAULT_INSTANCE)
    {
        ...
        $arrCart['subtotal'] = gc_currency_render($cart->subtotal());
        $arrCart['items'] = [];
        if ($cart->count()) {
            foreach ($cart->content() as $key => $item) {
                $product = \Glumbo\Gracart\Front\Models\ShopProduct::find($item->id);
                if ($product) {
                    $arrCart['items'][] = [
                        'id'        => $item->id,
                        'rowId'     => $item->rowId,
                        ...
                        'qty'       => $item->qty,
                        'image'     => gc_file($product->getThumb()),
