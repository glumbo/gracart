...
if (!function_exists('gc_clear_cache') && !in_array('gc_clear_cache', config('helper_except', []))) {
    /**
     * Clear cache
     *
     * @param   [string]  $domain
     ...
     */
    function gc_clear_cache($typeCache = 'cache_all', $storeId = null)
    {
        try {
...
if (!function_exists('gc_set_cache') && !in_array('gc_set_cache', config('helper_except', []))) {
    /**
     * [gc_set_cache description]
     *
     ...
     */
    function gc_set_cache($cacheIndex, $value, $time = null)
    {
        if (empty($cacheIndex)) {
            return ;
        }
        $seconds = $time ?? (gc_config_global('cache_time') ?? 600);
