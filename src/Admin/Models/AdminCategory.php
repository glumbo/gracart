...
class AdminCategory extends ShopCategory
{
    ...
    public static function getListTitleAdmin()
    {
        ...
        if (gc_config_global('cache_status') && gc_config_global('cache_category')) {
            if (!Cache::has($storeCache.'_cache_category_'.gc_get_locale())) {
                ...
                gc_set_cache($storeCache.'_cache_category_'.gc_get_locale(), self::$getListTitleAdmin);
            }
            return Cache::get($storeCache.'_cache_category_'.gc_get_locale());
        } else {
            if (self::$getListTitleAdmin === null) {
                self::$getListTitleAdmin = self::join($tableDescription, $tableDescription.'.category_id', $table.'.id')
                ->where('lang', gc_get_locale())
