...
use Glumbo\Gracart\Front\Models\ShopNewsStore;
...
class AdminNews extends ShopNews
{
    ...
    public static function getListTitleAdmin($storeId = null)
    {
        ...
        if (gc_config_global('cache_status') && gc_config_global('cache_news')) {
            if (!Cache::has($storeCache.'_cache_news_'.gc_get_locale())) {
                ...
                gc_set_cache($storeCache.'_cache_news_'.gc_get_locale(), self::$getListTitleAdmin);
            }
            return Cache::get($storeCache.'_cache_news_'.gc_get_locale());
        } else {
            if (self::$getListTitleAdmin === null) {
                $data = self::join($tableDescription, $tableDescription.'.news_id', $table.'.id')
                ->where('lang', gc_get_locale());
