...
class AdminProduct extends ShopProduct
{
    ...
    {
        return gc_product_admin_select_list($dataFilter, $storeId);
    }
    ...
    {
        $tableProductStore = (new ShopProductStore)->getTable();
        $storeId = $storeId ? gc_clean($storeId) : session('adminStoreId');
        $type = $type ? gc_clean($type) : 'sku';
        $fieldValue = gc_clean($fieldValue);
        $pId = gc_clean($pId);
        $check =  $this
    ...
    public function renderAttributeDetailsAdmin($currency = null, $rate = null)
    {
        ...
        foreach ($details as $groupId => $detailsGroup) {
            ...
            foreach ($detailsGroup as $k => $detail) {
                ...
                $html .= '<label class="radio-inline"><input ' . (($k == 0) ? "checked" : "") . ' type="radio" name="add_att[' . $this->id . '][' . $groupId . ']" value="' . $valueOption . '">' . gc_render_option_price($valueOption, $currency, $rate) . '</label> ';
            }
        }
        return $html;
    }
