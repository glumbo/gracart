...
class AdminUser extends Authenticatable
{
    ...
    public $table      = GC_DB_PREFIX.'admin_user';
    protected $guarded = [];
    protected $hidden  = [
    ...
    {
        return $this->belongsToMany(AdminRole::class, GC_DB_PREFIX.'admin_role_user', 'user_id', 'role_id');
    }
    ...
    {
        return $this->belongsToMany(AdminPermission::class, GC_DB_PREFIX.'admin_user_permission', 'user_id', 'permission_id');
    }
    ...
    {
        $dataUpdate = gc_clean($dataUpdate);
        $obj        = self::find($id);
    ...
    protected static function boot()
    {
        ...
        static::deleting(function ($model) {
            if (in_array($model->id, GC_GUARD_ADMIN)) {
                return false;
            }
            $model->roles()->detach();
        ...
        static::creating(function ($model) {
            if (empty($model->{$model->getKeyName()})) {
                $model->{$model->getKeyName()} = gc_generate_id($type = 'admin_user');
            }
        });
    }
    ...
    {
        $dataInsert = gc_clean($dataInsert);
            ...
            if (self::allPermissions()->first(function ($permission) {
                ...
                foreach ($actions as $key => $action) {
                    ...
                    if (
                        ...
                        && (
                            GC_ADMIN_PREFIX . '/config/*' == $method[1]
                        || GC_ADMIN_PREFIX . '/config/update_info' == $method[1]
                        || GC_ADMIN_PREFIX . '/config' == $method[1]
                        )
                    ) {
                        return true;
    ...
    public function sendPasswordResetNotification($token)
    {
        ...
        return gc_admin_sendmail_reset_notification($token, $emailReset);
    }
}
