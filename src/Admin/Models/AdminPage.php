...
use Cache;
use Glumbo\Gracart\Front\Models\ShopPageDescription;
use Glumbo\Gracart\Front\Models\ShopPageStore;
...
class AdminPage extends ShopPage
{
    ...
    public static function getListTitleAdmin($storeId = null)
    {
        ...
        if (gc_config_global('cache_status') && gc_config_global('cache_page')) {
            if (!Cache::has($storeCache.'_cache_page_'.gc_get_locale())) {
                if (self::$getListTitleAdmin === null) {
                    $data = self::join($tableDescription, $tableDescription.'.page_id', $table.'.id')
                    ->where('lang', gc_get_locale());
                    if ($storeId) {
                        $tablePageStore = (new ShopPageStore)->getTable();
                ...
                gc_set_cache($storeCache.'_cache_page_'.gc_get_locale(), self::$getListTitleAdmin);
            }
            return Cache::get($storeCache.'_cache_page_'.gc_get_locale());
        } else {
            if (self::$getListTitleAdmin === null) {
                $data = self::join($tableDescription, $tableDescription.'.page_id', $table.'.id')
                ->where('lang', gc_get_locale());
