...
class AdminPageController extends RootAdminController
{
    ...
    {
        $data = [
            'title'         => gc_language_render('admin.page.list'),
            'subTitle'      => '',
            'icon'          => 'fa fa-indent',
            'urlDeleteItem' => gc_route_admin('admin_page.delete'),
        ...
        $listTh = [
            'title'  => gc_language_render('admin.page.title'),
            'image'  => gc_language_render('admin.page.image'),
            'alias'  => gc_language_render('admin.page.alias'),
            'status' => gc_language_render('admin.page.status'),
        ];
        ...
        if (gc_check_multi_shop_installed() && session('adminStoreId') == GC_ID_ROOT) {
            ...
            // Only show store info if store is root
            if (function_exists('gc_get_list_store_of_page')) {
                $dataStores = gc_get_list_store_of_page($arrId);
            } else {
                $dataStores = [];
            }
        }
        ...
        //menuRight
        $data['menuRight'][] = '<a href="' . gc_route_admin('admin_page.create') . '" class="btn btn-sm btn-light btn-active-primary" title="New" id="button_create_new">
        ...
        //menuSearch
        $data['topMenuRight'][] = '
                <form action="' . gc_route_admin('admin_page.index') . '" id="button_search">
                <div class="input-group input-group" style="width: 350px;">
                    <select class="form-control rounded-0 select2" name="sort_order" id="sort_order">
                    '.$optionSort.'
                    </select> &nbsp;
                    <input type="text" name="keyword" class="form-control rounded-0 float-right" placeholder="' . gc_language_render('admin.page.search_place') . '" value="' . $keyword . '">
    ...
    public function create()
    {
        $page = [];
        $data = [
            'title'             => gc_language_render('admin.page.add_new_title'),
            'subTitle'          => '',
            'title_description' => gc_language_render('admin.page.add_new_des'),
            'icon'              => 'fa fa-plus',
            'languages'         => $this->languages,
            'page'              => $page,
            'url_action'        => gc_route_admin('admin_page.create'),
    ...
    public function postCreate()
    {
        ...
        $validator = Validator::make(
            $data,$arrValidation,
            [
                'alias.regex' => gc_language_render('admin.page.alias_validate'),
        ...
        $fields = $data['fields'] ?? [];
        gc_update_custom_field($fields, $page->id, 'shop_page');

        gc_clear_cache('cache_page');
        return redirect()->route('admin_page.index')->with('success', gc_language_render('action.create_success'));
    }
    ...
    public function edit($id)
    {
        ...
        $data = [
            ...
            'languages' => $this->languages,
            'page' => $page,
            'url_action' => gc_route_admin('admin_page.edit', ['id' => $page['id']]),
            'customFields'      => (new ShopCustomField)->getCustomField($type = 'shop_page'),
        ];
        return view($this->templatePathAdmin.'screen.page')
            ->with($data);
    ...
    public function postEdit($id)
    {
        ...
        gc_clear_cache('cache_page');
        return redirect()->route('admin_page.index')->with('success', gc_language_render('action.edit_success'));
    }
