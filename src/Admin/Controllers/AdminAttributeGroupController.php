...
class AdminAttributeGroupController extends RootAdminController
{
    ...
    {
        $data = [
            'title' => gc_language_render('admin.attribute_group.list'),
            'title_action' => '<i class="fa fa-plus" aria-hidden="true"></i> ' . gc_language_render('action.add'),
            'subTitle' => '',
            'icon' => 'fa fa-indent',
            'urlDeleteItem' => gc_route_admin('admin_attribute_group.delete'),
            'removeList' => 0, // 1 - Enable function delete list item
            'buttonRefresh' => 0, // 1 - Enable button refresh
            'css' => '',
            'js' => '',
            'url_action' => gc_route_admin('admin_attribute_group.create'),
        ];

        $listTh = [
            'name' => gc_language_render('admin.attribute_group.name'),
            'type' => gc_language_render('admin.attribute_group.type'),
            'action' => gc_language_render('action.create'),
        ];
        ...
        foreach ($dataTmp as $key => $row) {
            $dataTr[$row['id']] = [
                ...
                'type' => $row['type'],
                'action' => '
                    <a href="' . gc_route_admin('admin_attribute_group.edit', ['id' => $row['id'] ? $row['id'] : 'not-found-id']) . '"><span title="' . gc_language_render('action.edit') . '" type="button" class="btn btn-sm btn-light btn-active-primary"><i class="fa fa-edit"></i></span></a>&nbsp;
    ...
    public function postCreate()
    {
        ...
        return redirect()->route('admin_attribute_group.index')->with('success', gc_language_render('action.create_success'));
    }
    ...
    public function edit($id)
    {
        ...
        'css' => '',
        'js' => '',
        'url_action' => gc_route_admin('admin_attribute_group.edit', ['id' => $attribute_group['id']]),
