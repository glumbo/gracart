...
use Glumbo\Gracart\Admin\Models\AdminLink;
...
class AdminStoreLinkController extends RootAdminController
{
    ...
    {
        $data = [
            'title' => gc_language_render('admin.link.list'),
            'subTitle' => '',
            'icon' => 'fa fa-indent',
            'urlDeleteItem' => gc_route_admin('admin_store_link.delete'),
        ...
        $listTh = [
            'name' => gc_language_render('admin.link.name'),
            'url' => gc_language_render('admin.link.url'),
            'collection' => gc_language_render('admin.link.collection'),
            'group' => gc_language_render('admin.link.group'),
            'sort' => gc_language_render('admin.link.sort'),
            'status' => gc_language_render('admin.link.status'),
        ];
        ...
        if (gc_check_multi_shop_installed() && session('adminStoreId') == GC_ID_ROOT) {
            ...
            if (function_exists('gc_get_list_store_of_link')) {
                $dataStores = gc_get_list_store_of_link($arrId);
            } else {
                $dataStores = [];
            }
        }
        ...
        foreach ($dataTmp as $key => $row) {
            ...
            $dataMap['action'] = '<a href="' . gc_route_admin('admin_store_link.edit', ['id' => $row['id'] ? $row['id'] : 'not-found-id']) . '"><span title="' . gc_language_render('action.edit') . '" type="button" class="btn btn-sm btn-light btn-active-primary"><i class="fa fa-edit"></i></span></a>&nbsp;
        ...
        //menuRight
        $data['menuRight'][] = '<a href="' . gc_route_admin('admin_store_link.create') . '" class="btn btn-sm btn-light btn-active-primary" title="New" id="button_create_new">
        <i class="fa fa-plus" title="' . gc_language_render('admin.link.add_new') . '"></i>
        </a>';
        $data['menuRight'][] = '<a href="' . gc_route_admin('admin_store_link.collection_create') . '" class="btn btn-sm btn-light btn-active-primary" title="'.gc_language_render('admin.link.add_collection_new').'" id="button_create_new">
    ...
    {
        $data = [
            'title'             => gc_language_render('admin.link.add_new_title'),
            'subTitle'          => '',
            'title_description' => gc_language_render('admin.link.add_new_des'),
            'icon'              => 'fa fa-plus',
            'link'              => [],
            ...
            'layout'            => 'single',
            'url_action'        => gc_route_admin('admin_store_link.create'),
        ];
    ...
    {
        $data = [
            'title'             => gc_language_render('admin.link.add_new_collection_title'),
            'subTitle'          => '',
            'title_description' => gc_language_render('admin.link.add_new_collection_des'),
            'icon'              => 'fa fa-plus',
            'link'              => [],
            'arrTarget'         => $this->arrTarget,
            'arrGroup'          => $this->arrGroup(),
            'arrCollection'           => $this->arrCollection(),
            'layout'            => 'collection',
            'url_action'        => gc_route_admin('admin_store_link.collection_create'),
        ];
        return view($this->templatePathAdmin.'screen.store_link')
            ->with($data);
    }
    ...
    public function postCreate()
    {
        ...
        return redirect()->route('admin_store_link.index')->with('success', gc_language_render('action.create_success'));
    }
    ...
    public function edit($id)
    {
        ...
        $data = [
            ...
            'url_action'        => gc_route_admin('admin_store_link.edit', ['id' => $link['id']]),
        ];
        return view($this->templatePathAdmin.'screen.store_link')
            ->with($data);
    }
    ...
    public function postEdit($id)
    {
        ...
        return redirect()->route('admin_store_link.index')->with('success', gc_language_render('action.edit_success'));
    }
