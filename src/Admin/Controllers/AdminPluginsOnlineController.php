...
class AdminPluginsOnlineController extends RootAdminController
{
    ...
    protected function pluginCode($code)
    {
        ...
        $url = config('grakan.api_link').'/plugins/'.$code.'?page[size]=20&page[number]='.$page;
        $url .='&version='.$gc_version;
        ...
        if (!empty($dataApi['data'])) {
            ...
            $resultItems = gc_language_render('admin.result_item', ['item_from' => $dataApi['from'] ?? 0, 'item_to' => $dataApi['to']??0, 'total' =>  $dataApi['total'] ?? 0]);
            $htmlPaging .= '<ul class="pagination pagination-sm no-margin pull-right">';
            if ($dataApi['current_page'] > 1) {
                $htmlPaging .= '<li class="page-item"><a class="page-link pjax-container" href="'.gc_route_admin('admin_plugin_online', ['code' => strtolower($code)]).'?page='.($dataApi['current_page'] - 1).'" rel="prev">«</a></li>';
            } else {
                for ($i = 1; $i < $dataApi['last_page']; $i++) {
                    if ($dataApi['current_page'] == $i) {
                        ...
                        $htmlPaging .= '<li class="page-item"><a class="page-link" href="'.gc_route_admin('admin_plugin_online', ['code' => strtolower($code)]).'?page='.$i.'">'.$i.'</a></li>';
                    }
                }
            }
            if ($dataApi['current_page'] < $dataApi['last_page']) {
                $htmlPaging .= '<li class="page-item"><a class="page-link pjax-container" href="'.gc_route_admin('admin_plugin_online', ['code' => strtolower($code)]).'?page='.($dataApi['current_page'] + 1).'" rel="next">»</a></li>';
            }
            $htmlPaging .= '</ul>';
        }
        $code = gc_word_format_class($code);

        $arrPluginLocal = gc_get_all_plugin($code);
    ...
    public function install()
    {
        ...
        try {
            ...
            $unzip = gc_unzip(storage_path('tmp/'.$pathTmp.'/'.$fileTmp), storage_path('tmp/'.$pathTmp));
            if ($unzip) {
                ...
                if (!gc_plugin_compatibility_check($scartVersion)) {
                    ...
                    $response = ['error' => 1, 'msg' => gc_language_render('admin.plugin.not_compatible', ['version' => $scartVersion, 'gc_version' => config('grakan.core')])];
                } else {
