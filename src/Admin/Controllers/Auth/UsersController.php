    ...
    public function index()
    {
        ...
        foreach ($dataTmp as $key => $row) {
            ...
            $dataTr[] = [
                ...
                'action' => '
                    <a href="' . gc_route_admin('admin_user.edit', ['id' => $row['id'] ? $row['id'] : 'not-found-id']) . '"><span title="' . gc_language_render('action.edit') . '" type="button" class="btn btn-flat btn-sm btn-primary"><i class="fa fa-edit"></i></span></a>&nbsp;
                    ' . ((Admin::user()->id == $row['id'] || in_array($row['id'], GC_GUARD_ADMIN)) ? '' : '<span onclick="deleteItem(\'' . $row['id'] . '\');"  title="' . gc_language_render('action.delete') . '" class="btn btn-flat btn-sm btn-danger"><i class="fas fa-trash-alt"></i></span>')
                ,
            ];
        }
        ...
        //menuRight
        $data['menuRight'][] = '<a href="' . gc_route_admin('admin_user.create') . '" class="btn  btn-success  btn-flat" title="New" id="button_create_new">
        ...
        $data['topMenuRight'][] = '
                ...
                <div class="input-group input-group" style="width: 350px;">
                    ...
                    <input type="text" name="keyword" class="form-control rounded-0 float-right" placeholder="' . gc_language_render('admin.user.search_place') . '" value="' . $keyword . '">
