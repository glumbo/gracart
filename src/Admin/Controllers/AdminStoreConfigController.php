...
class AdminStoreConfigController extends RootAdminController
{
    ...
    public function index()
    {
        ...
        $data = [
            'title' => gc_language_render('admin.menu_titles.config_store_default'),
            'subTitle' => '',
            'icon' => 'fas fa-cogs',
        ];
        ...
        $taxs[0] = gc_language_render('admin.tax.non_tax');
        ...
        $data['captcha_page'] = [
            'register' => gc_language_render('admin.captcha.captcha_page_register'),
            'forgot'   => gc_language_render('admin.captcha.captcha_page_forgot_password'),
            'checkout' => gc_language_render('admin.captcha.captcha_page_checkout'),
            'contact'  => gc_language_render('admin.captcha.captcha_page_contact'),
            'review'   => gc_language_render('admin.captcha.captcha_page_review'),
        ];
        ...
        $data['pluginCaptchaInstalled']         = gc_get_plugin_captcha_installed();
        ...
        $data['storeId']                        = $id;
        $data['urlUpdateConfig']                = gc_route_admin('admin_config.update');
    ...
    */
    ...
    public function addNew() {
        ...
        if (session('adminStoreId') != GC_ID_ROOT && $storeId != session('adminStoreId')) {
            return response()->json(['error' => 1, 'msg' => gc_language_render('admin.remove_dont_permisison') . ': storeId#' . $storeId]);
        }

        if (!$key) {
            return redirect()->back()->with('error', 'Key: '.gc_language_render('admin.not_empty'));
        }
        ...
        if (AdminConfig::where(['key' => $key, 'store_id' => $storeId])->first()) {
            return redirect()->back()->with('error', gc_language_quickly('admin.admin_custom_config.key_exist', 'Key already exist'));
        }
        $dataUpdate = gc_clean($dataUpdate, [], true);
        AdminConfig::insert($dataUpdate);
        return redirect()->back()->with('success', gc_language_render('action.update_success'));
    }
