...
class AdminStoreInfoController extends RootAdminController
{
    ...
    */
        ...
        if (!$lang) {
            try {
                if ($name == 'type') {
                    ...
                    $error = 1;
                    $msg = gc_language_render('store.admin.value_cannot_change');
                } elseif ($name == 'domain') {
                    if (
                        $storeId == GC_ID_ROOT
                        || ((gc_check_multi_vendor_installed()) && gc_store_is_partner($storeId))
                        || gc_check_multi_store_installed()
                    ) {
                        // Only store root can edit domain
                        $domain = gc_process_domain_store($value);
                        if (AdminStore::where('domain', $domain)->where('id', '<>', $storeId)->first()) {
                            $error = 1;
                            $msg = gc_language_render('store.admin.domain_exist');
                        } else {
                    ...
                    if (AdminStore::where('code', $value)->where('id', '<>', $storeId)->first()) {
                        $error = 1;
                        $msg = gc_language_render('store.admin.code_exist');
                    } else {
                    ...
                    if (file_exists($fileProcess = resource_path() . '/views/templates/'.$value.'/Provider.php')) {
                        ...
                        if (function_exists('gc_template_install_store')) {
                            ...
                            $checkTemplateEnableStore = (new \Glumbo\Gracart\Front\Models\ShopStoreCss)
    ...
    public function index()
    {
        ...
        if (!$store) {
            $data = [
                'title' => gc_language_render('store.admin.title'),
                'subTitle' => '',
                'icon' => 'fas fa-cogs',
                'dataNotFound' => 1
            ];
