...
class AdminOrderStatusController extends RootAdminController
{
    ...
    {
        $data = [
            'title' => gc_language_render('admin.order_status.list'),
            'title_action' => '<i class="fa fa-plus" aria-hidden="true"></i> ' . gc_language_render('admin.order_status.add_new_title'),
            'subTitle' => '',
            'icon' => 'fa fa-indent',
            'urlDeleteItem' => gc_route_admin('admin_order_status.delete'),
            'removeList' => 0, // 1 - Enable function delete list item
            'buttonRefresh' => 0, // 1 - Enable button refresh
            'css' => '',
            'js' => '',
            'url_action' => gc_route_admin('admin_order_status.create'),
        ];

        $listTh = [
            'id' => 'ID',
            'name' => gc_language_render('admin.order_status.name'),
        ...
        foreach ($dataTmp as $key => $row) {
            $dataTr[$row['id']] = [
                ...
                'name' => $row['name'] ?? 'N/A',
                'action' => '
                    <a href="' . gc_route_admin('admin_order_status.edit', ['id' => $row['id'] ? $row['id'] : 'not-found-id']) . '"><span title="' . gc_language_render('action.edit') . '" type="button" class="btn btn-sm btn-light btn-active-primary"><i class="fa fa-edit"></i></span></a>&nbsp;
    ...
    public function postCreate()
    {
        ...
        return redirect()->route('admin_order_status.edit', ['id' => $obj['id']])->with('success', gc_language_render('action.create_success'));
    }
    ...
    public function edit($id)
    {
        ...
        'css' => '',
        'js' => '',
        'url_action' => gc_route_admin('admin_order_status.edit', ['id' => $order_status['id']]),
