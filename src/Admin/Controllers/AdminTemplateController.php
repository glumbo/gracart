...
class AdminTemplateController extends RootAdminController
{
    ...
    {
        $data = [
            'title' => gc_language_render('admin.template.list'),
            'subTitle' => '',
            'icon' => 'fa fa-indent',        ];

        $data["templates"] = gc_get_all_template();
    ...
    public function remove()
    {
        ...
        if (file_exists($fileProcess = resource_path() . '/views/templates/'.$key.'/Provider.php')) {
            ...
            if (function_exists('gc_template_uninstall')) {
                // Remove template from all stories
                gc_template_uninstall();
    ...
    public function refresh()
    {
        ...
        if (file_exists($fileProcess = resource_path() . '/views/templates/'.$key.'/Provider.php')) {
            ...
            if (function_exists('gc_template_uninstall') && function_exists('gc_template_install')) {
                //Remove all stories
                gc_template_uninstall();
                //Install data default and data for root domain
                gc_template_install($data);
            }
        }
    ...
    public function disable()
    {
        ...
        $response = ['error' => 0, 'msg' => gc_language_render('action.disable_success')];
        return response()->json($response);
    }
    ...
    public function enable()
    {
        ...
        $response = ['error' => 0, 'msg' => gc_language_render('action.enable_success')];
        return response()->json($response);
    }
    ...
    {
        $data =  [
            'title' => gc_language_render('admin.template.import')
        ];
    ...
    public function processImport()
    {
        ...
        $validator = \Validator::make(
            $data,
            [
                'file'   => 'required|mimetypes:application/zip|max:'.min($maxSizeConfig = gc_getMaximumFileUploadSize($unit = 'K'), 51200),
            ]
        );
        ...
        $pathTmp = time();
        $pathFile = gc_file_upload($data['file'], 'tmp', $pathFolder = $pathTmp)['pathFile'] ?? '';
        ...
        if ($pathFile) {
            ...
            $unzip = gc_unzip(storage_path('tmp/'.$pathFile), storage_path('tmp/'.$pathTmp));
            if ($unzip) {
                ...
                if ($checkConfig) {
                    ...
                    if (!gc_plugin_compatibility_check($scartVersion)) {
                        File::deleteDirectory(storage_path('tmp/'.$pathTmp));
                        return response()->json(['error' => 1, 'msg' => gc_language_render('admin.plugin.not_compatible', ['version' => $scartVersion, 'gc_version' => config('grakan.core')])]);
                    }
                    ...
                    if (!$configKey) {
                        File::deleteDirectory(storage_path('tmp/'.$pathTmp));
                        return redirect()->back()->with('error', gc_language_render('admin.template.error_config'));
                    }

                    $arrTemplateLocal = gc_get_all_template();
                    if (array_key_exists($configKey, $arrTemplateLocal)) {
                        File::deleteDirectory(storage_path('tmp/'.$pathTmp));
                        return redirect()->back()->with('error', gc_language_render('admin.template.error_exist'));
                    }
                    try {
                        ...
                        if (file_exists($fileProcess = resource_path() . '/views/templates/'.$configKey.'/Provider.php')) {
                            ...
                            if (function_exists('gc_template_install')) {
                                //Install data default and data for root domain
                                gc_template_install($data);
                            }
                        }
                    ...
                    File::deleteDirectory(storage_path('tmp/'.$pathTmp));
                    return redirect()->back()->with('error', gc_language_render('admin.template.error_check_config'));
                }
            } else {
                return redirect()->back()->with('error', gc_language_render('admin.template.error_unzip'));
            }
        } else {
            return redirect()->back()->with('error', gc_language_render('admin.template.error_upload'));
        }
        
        if (count($config)) {
            (new AdminTemplate)->create(['key' => $config['configKey'] ?? '', 'name' => $config['name'] ?? '', 'status' => 1]);
        }
        return redirect()->route('admin_template.index')->with('success', gc_language_render('admin.template.import_success'));
    }
}
