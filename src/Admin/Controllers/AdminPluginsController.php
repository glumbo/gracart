...
class AdminPluginsController extends RootAdminController
{
    ...
    public function index($code)
    {
        ...
        if ($action == 'config' && $pluginKey != '') {
            $namespace = gc_get_class_plugin_config($code, $pluginKey);
    ...
    protected function pluginCode($code)
    {
        ...
        $pluginsInstalled = gc_get_plugin_installed($code, $onlyActive = false);
        $plugins = gc_get_all_plugin($code);
        $title = gc_language_render('admin.plugin.' . $code.'_plugin');
    ...
    public function install()
    {
        $key = request('key');
        $code = request('code');
        $namespace = gc_get_class_plugin_config($code, $key);
    ...
    {
        $data =  [
            'title' => gc_language_render('admin.plugin.import')
        ];
            ...
            if ($unzip) {
                ...
                if ($checkConfig) {
                    ...
                    if (!gc_plugin_compatibility_check($scartVersion)) {
                        ...
                        return redirect()->back()->with('error', gc_language_render('admin.plugin.not_compatible', ['version' => $scartVersion, 'gc_version' => config('grakan.core')]));
                    }
                    ...
                    if (!$configGroup || !$configCode || !$configKey) {
                        File::deleteDirectory(storage_path('tmp/'.$pathTmp));
                        return redirect()->back()->with('error', gc_language_render('admin.plugin.error_config_format'));
                    }
                    //Check plugin exist
                    $arrPluginLocal = gc_get_all_plugin($configCode);
                    if (array_key_exists($configKey, $arrPluginLocal)) {
                        File::deleteDirectory(storage_path('tmp/'.$pathTmp));
                        return redirect()->back()->with('error', gc_language_render('admin.plugin.error_exist'));
                    }
                    ...
                    try {
                        ...
                        File::deleteDirectory(storage_path('tmp/'.$pathTmp));
                        $namespace = gc_get_class_plugin_config($configCode, $configKey);
                        $response = (new $namespace)->install();
                    ...
                    File::deleteDirectory(storage_path('tmp/'.$pathTmp));
                    return redirect()->back()->with('error', gc_language_render('admin.plugin.error_check_config'));
                }
            } else {
                return redirect()->back()->with('error', gc_language_render('admin.plugin.error_unzip'));
            }
        } else {
            return redirect()->back()->with('error', gc_language_render('admin.plugin.error_upload'));
        }
        if ($linkRedirect) {
            return redirect($linkRedirect)->with('success', gc_language_render('admin.plugin.import_success'));
        } else {
            return redirect()->back()->with('success', gc_language_render('admin.plugin.import_success'));
        }
    }
}
