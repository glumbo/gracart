...
use Glumbo\Gracart\Admin\Models\AdminMenu;
...
class AdminMenuController extends RootAdminController
{
    ...
    {
        $data = [
            'title' => gc_language_render('admin.menu.list'),
            'subTitle' => '',
            'icon' => 'fa fa-indent',
            'menu' => [],
            'treeMenu' => (new AdminMenu)->getTree(),
            'url_action' => gc_route_admin('admin_menu.create'),
            'urlDeleteItem' => gc_route_admin('admin_menu.delete'),
            'title_form' => '<i class="fa fa-plus" aria-hidden="true"></i> ' . gc_language_render('admin.menu.create'),
        ];
        $data['layout'] = 'index';
    ...
    public function postCreate()
    {
        ...
        return redirect()->route('admin_menu.index')->with('success', gc_language_render('admin.menu.create_success'));
    }
    ...
    public function edit($id)
    {
        ...
        $data = [
            ...
            'menu' => $menu,
            'treeMenu' => (new AdminMenu)->getTree(),
            'url_action' => gc_route_admin('admin_menu.edit', ['id' => $menu['id']]),
            'title_form' => '<i class="fa fa-edit" aria-hidden="true"></i> ' . gc_language_render('admin.menu.edit'),
        ];
        $data['urlDeleteItem'] = gc_route_admin('admin_menu.delete');
        $data['id'] = $id;
        $data['layout'] = 'edit';
        return view($this->templatePathAdmin.'screen.list_menu')
            ->with($data);
    ...
    public function postEdit($id)
    {
        ...
        return redirect()->back()->with('success', gc_language_render('admin.menu.edit_success'));
    }
    ...
    public function deleteList()
    {
        if (!request()->ajax()) {
            ...
            if ($check) {
                return response()->json(['error' => 1, 'msg' => gc_language_render('admin.menu.error_have_child')]);
            } else {
