...
class AdminBackupController extends RootAdminController
{
    ...
    {
        $tableInfo = DB::connection(GC_CONNECTION)
            ->table('INFORMATION_SCHEMA.TABLES')
            ->select(['TABLE_NAME', 'TABLE_ROWS', 'DATA_LENGTH'])
            ->where('TABLE_SCHEMA', config('database.connections.'.GC_CONNECTION.'.database'))
        ...
        return view($this->templatePathAdmin.'screen.backup')->with(
            [
                "title"    => gc_language_render('admin.backup.title'),
                "arrFiles" => $arrFiles,
                "tableInfo" => $tableInfo,
            ]
        )->render();
    ...
    public function processBackupFile()
    {
        ...
        if ($action === 'remove') {
            try {
                unlink($pathFull);
                $return = ['error' => 0, 'msg' => gc_language_render('action.remove_success')];
            } catch (\Throwable $e) {
