...
class AdminCategoryController extends RootAdminController
{
    ...
    public function index()
    {
        ...
        $data = [
            'title'         => gc_language_render('admin.category.list'),
            'subTitle'      => '',
            'icon'          => 'fa fa-indent',
            'urlDeleteItem' => gc_route_admin('admin_category.delete'),
        ...
        $listTh = [
            'image'  => gc_language_render('admin.category.image'),
            'title'  => gc_language_render('admin.category.title'),
            'parent' => gc_language_render('admin.category.parent'),
            'top'    => gc_language_render('admin.category.top'),
            'status' => gc_language_render('admin.category.status'),
            'sort'   => gc_language_render('admin.category.sort'),
        ];

        if (gc_check_multi_store_installed() && session('adminStoreId') == GC_ID_ROOT) {
        ...
        if (gc_check_multi_store_installed() && session('adminStoreId') == GC_ID_ROOT) {
            ...
            if (function_exists('gc_get_list_store_of_category')) {
                $dataStores = gc_get_list_store_of_category($arrId);
            } else {
                $dataStores = [];
            }
        }
        ...
        foreach ($dataTmp as $key => $row) {
            ...
            $dataMap['action'] = '
            <a href="' . gc_route_admin('admin_category.edit', ['id' => $row['id'] ? $row['id'] : 'not-found-id']) . '"><span title="' .gc_language_render('action.edit') . '" type="button" class="btn btn-sm btn-light btn-active-primary"><i class="fa fa-edit"></i></span></a>&nbsp;
            ...
            <a target=_new href="' . gc_route('category.detail', ['alias' => $row['alias']]) . '"><span title="Link" type="button" class="btn btn-sm btn-light btn-active-warning"><i class="fas fa-external-link-alt"></i></a>';
        ...
        //menuSearch
        $data['topMenuRight'][] = '
                <form action="' . gc_route_admin('admin_category.index') . '" id="button_search">
    ...
    {
        $data = [
            'title' => gc_language_render('admin.category.add_new_title'),
            'subTitle' => '',
            'title_description' => gc_language_render('admin.category.add_new_des'),
            'icon' => 'fa fa-plus',
            'languages' => $this->languages,
            'category' => [],
            'categories' => (new AdminCategory)->getTreeCategoriesAdmin(),
            'url_action' => gc_route_admin('admin_category.create'),
    ...
    public function postCreate()
    {
        ...
        $validator = Validator::make(
            $data,$arrValidation,
            [
                'descriptions.*.title.required' => gc_language_render('validation.required', ['attribute' => gc_language_render('admin.category.title')]),
                'alias.regex' => gc_language_render('admin.category.alias_validate'),
            ]
        );
        ...
        $fields = $data['fields'] ?? [];
        gc_update_custom_field($fields, $category->id, 'shop_category');

        gc_clear_cache('cache_category');

        return redirect()->route('admin_category.index')->with('success', gc_language_render('action.create_success'));
    }
    ...
    public function edit($id)
    {
        ...
        $data = [
            ...
            'category'          => $category,
            'categories'        => (new AdminCategory)->getTreeCategoriesAdmin(),
            'url_action'        => gc_route_admin('admin_category.edit', ['id' => $category['id']]),
            'customFields'      => (new ShopCustomField)->getCustomField($type = 'shop_category'),
        ];
        return view($this->templatePathAdmin.'screen.category')
            ->with($data);
    ...
    public function postEdit($id)
    {
        ...
        //
        return redirect()->route('admin_category.index')->with('success', gc_language_render('action.edit_success'));
    }
