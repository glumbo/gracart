<span class="{{ isset($size) && $size == "x" ? '' : 'symbol symbol-50px me-2' }}">
<span class="symbol-label {{ isset($size) && $size == "x" ? '' : 'bg-light-' }}{{ $color ?? 'primary' }}">
	<!--begin::Svg Icon | path: icons/duotune/finance/fin006.svg-->
	<span class="svg-icon svg-icon-{{ $size ?? '3x' }} svg-icon-{{ $color ?? 'primary' }}">
