    ...
    <div class="card">
      <div class="card-header">
        ...
        <div class="btn-group float-right" style="margin-right: 5px">
            <a href="{{ gc_route_admin('admin_language.index') }}" class="btn  btn-flat btn-default" title="List"><i class="fa fa-list"></i><span class="hidden-xs"> {{ gc_language_render('admin.back_list') }}</span></a>
        </div>
      @endif
      </div>
      <!-- /.card-header -->
      ...
      <form action="{{ $url_action }}" method="post" accept-charset="UTF-8" class="form-horizontal" id="form-main">
        <div class="card-body">
          ...
          <div class="form-group row {{ $errors->has('name') ? ' text-red' : '' }}">
            <label for="name" class="col-sm-2 col-form-label">{{ gc_language_render('admin.language.name') }}</label>
            <div class="col-sm-10 ">
          ...
          <div class="form-group row {{ $errors->has('code') ? ' text-red' : '' }}">
            <label for="code" class="col-sm-2 col-form-label">{!! gc_language_render('admin.language.code') !!}</label>
            <div class="col-sm-10 ">
              <div class="input-group mb-3">
                <div class="input-group-prepend">
                  <span class="input-group-text"><i class="fas fa-pencil-alt"></i></span>
          ...
          <div class="form-group row {{ $errors->has('icon') ? ' text-red' : '' }}">
            <label for="icon" class="col-sm-2 col-form-label">{!! gc_language_render('admin.language.icon') !!}</label>
            <div class="col-sm-10 ">
              <div class="input-group mb-3">
                <div class="input-group-prepend">
                  <span class="input-group-text"><i class="fas fa-pencil-alt"></i></span>
                ...
                <div class="input-group-append">
                  <span data-input="icon" data-preview="preview_icon" data-type="language"
                      class="btn btn-primary lfm"><i class="fa fa-icon"></i>  {{gc_language_render('admin.choose_icon')}}</span>
                </div>
              </div>
              ...
              <div id="preview_icon" class="img_holder"><img
                src="{{ gc_file(old('icon',$language['icon']??'images/no-image.jpg')) }}">
              </div>
            </div>
          </div>
          ...
          <div class="form-group row {{ $errors->has('rtl') ? ' text-red' : '' }}">
            <label for="rtl" class="col-sm-2 col-form-label">{!! gc_language_render('admin.language.layout_rtl') !!}</label>
            <div class="col-sm-10 ">
              <div class="input-group mb-3">
          ...
          <div class="form-group row {{ $errors->has('sort') ? ' text-red' : '' }}">
            <label for="sort" class="col-sm-2 col-form-label">{!! gc_language_render('admin.language.sort') !!}</label>
            <div class="col-sm-10 ">
              <div class="input-group mb-3">
          ...
          <div class="form-group row {{ $errors->has('status') ? ' text-red' : '' }}">
            <label for="status" class="col-sm-2 col-form-label">{!! gc_language_render('admin.language.status') !!}</label>
            <div class="col-sm-10 ">
              <div class="input-group mb-3">
            ...
            $.ajax({
                ...
                success: function (data) {
                    if(data.error == 1){
                      ...
                      window.location.replace('{{ gc_route_admin('admin_language.index') }}');
                    }
