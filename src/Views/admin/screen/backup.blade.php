          ...
          <div class="card">
            ...
            <div class="card-body"  id="pjax-container">
              ...
              <div class="btn-group">
                ...
                <button type="button" class="btn btn-default btn-flat dropdown-toggle" data-toggle="dropdown" data-loading-text="{{ gc_language_render('admin.backup.processing') }}">
              ...
              <table id="main-table" class="table table-hover box-body text-wrap table-bordered">
                <thead>
                <tr>
                  <th>{{ gc_language_render('admin.backup.sort') }}</th>
                  <th>{{ gc_language_render('admin.backup.date') }}</th>
                  <th>{{ gc_language_render('admin.backup.name') }}</th>
                  <th>{{ gc_language_render('admin.backup.size') }}</th>
                  <th>{{ gc_language_render('admin.backup.download') }}</th>
                  <th>{{ gc_language_render('admin.backup.remove') }}</th>
                  <th>{{ gc_language_render('admin.backup.restore') }}</th>
                </tr>
                </thead>
                <tbody>
                  @php
...
<script type="text/javascript">
      ...
      Swal.fire({
        ...
        if (result.value) {
          ...
          $.ajax({
            type: 'POST',
            dataType:'json',
            url: '{{ gc_route_admin('admin.backup.process') }}',
            data: {
              "_token": "{{ csrf_token() }}",
              "file":file,
              "action":action,
  ...
  function generateBackup(obj) {
      ...
      $.ajax({
        type: 'POST',
        dataType:'json',
        url: '{{ gc_route_admin('admin.backup.generate') }}',
        data: {
          "_token": "{{ csrf_token() }}",
