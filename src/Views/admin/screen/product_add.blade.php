                ...
                <div id="main-add" class="card-body">
                        ...
                        <div class="form-group row kind  {{ $errors->has('descriptions.'.$code.'.content') ? ' text-red' : '' }}">
                            <label for="{{ $code }}__content" class="col-sm-2 col-form-label">
                                {{ gc_language_render('product.content') }}
                            </label>
                            <div class="col-sm-8">
...
@endif
                        ...
                        <div class="form-group row kind  {{ $errors->has('image') ? ' text-red' : '' }}">
                            <label for="image" class="col-sm-2 col-form-label">
                                {{ gc_language_render('product.image') }}
                            </label>
                            <div class="col-sm-8">
                                <div class="input-group">
                                    ...
                                    <div class="input-group-append">
                                        <span class="btn btn-primary lfm" data-input="image" data-preview="preview_image" data-type="product">
                                            <i class="fas fa-image"></i> {{gc_language_render('product.admin.choose_image')}}
                                        </span>
                                    </div>
                                </div>
                                @if ($errors->has('image'))
...
@if (gc_config_admin('product_brand'))
                        ...
                        <div class="form-group row kind   {{ $errors->has('brand_id') ? ' text-red' : '' }}">
                            <label for="brand_id"
                                class="col-sm-2 col-form-label">{{ gc_language_render('product.brand') }}</label>
                            <div class="col-sm-8">
                                <div class="input-group">
...
@if (gc_config_admin('product_supplier'))
                        ...
                        <div class="form-group row kind   {{ $errors->has('supplier_id') ? ' text-red' : '' }}">
                            <label for="supplier_id"
                                class="col-sm-2 col-form-label">{{ gc_language_render('product.supplier') }}</label>
                            <div class="col-sm-8">
                                <div class="input-group">
...
@if (gc_config_admin('product_tax'))
                        ...
                        <div class="form-group row kind   {{ $errors->has('tax_id') ? ' text-red' : '' }}">
                            <label for="tax_id"
                                class="col-sm-2 col-form-label">{{ gc_language_render('product.tax') }}
                            </label>
                            <div class="col-sm-8">
                                <div class="input-group">
                                <select class="form-control input-sm tax_id select2"
                                    name="tax_id">
                                    <option value="0" {{ (old('tax_id') == 0) ? 'selected':'' }}>{{ gc_language_render('admin.tax.non_tax') }}</option>
                                    <option value="auto" {{ (old('tax_id') == 'auto') ? 'selected':'' }}>{{ gc_language_render('admin.tax.auto') }}</option>
                                    @foreach ($taxs as $k => $v)
                        ...
                        <div class="form-group row kind   {{ $errors->has('property') ? ' text-red' : '' }}">
                            ...
                            <div class="col-sm-8">
                                ...
                                <div class="icheck-primary d-inline">
                                    <input type="radio" id="radioPrimary{{ $key }}" name="property" value="{{ $key }}" {{ ((!old() && $key == GC_PROPERTY_PHYSICAL) || old('property') == $key)?'checked':'' }}>
                                ...
                                @endif
                                <div style="margin-top: 10px; {{ (old('property') != GC_PROPERTY_DOWNLOAD) ? 'display:none':'' }}" id="download_path">
                                    <input type="text"  name="download_path" value="{{ old('download_path') }}" class="form-control input-sm" placeholder="{{ gc_language_render('product.download_path') }}" />
                                </div>
                            </div>
                        </div>
                        {{-- //property --}}
