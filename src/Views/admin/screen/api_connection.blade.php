        ...
        <div class="card">
          <div class="card-header">
            ...
            <div class="btn-group float-right" style="margin-right: 5px">
                <a href="{{ gc_route_admin('admin_api_connection.index') }}" class="btn btn-flat btn-default" title="List"><i class="fa fa-list"></i>
                  <span class="hidden-xs"> {{ gc_language_render('admin.back_list') }}</span>
                </a>
            </div>
          @endif
          </div>
          ...
          <form action="{{ $url_action }}" method="post" accept-charset="UTF-8" class="form-horizontal" id="form-main">
            <div class="card-body">
              ...
              <div class="form-group row {{ $errors->has('description') ? ' text-red' : '' }}">
                <label for="description" class="col-sm-2 col-form-label">{{ gc_language_render('admin.api_connection.description') }}</label>
                <div class="col-sm-10 ">
              ...
              <div class="form-group row {{ $errors->has('apiconnection') ? ' text-red' : '' }}">
                <label for="apiconnection" class="col-sm-2 col-form-label">{{ gc_language_render('admin.api_connection.connection') }}</label>
                <div class="col-sm-10 ">
                  <div class="input-group mb-3">
                    <div class="input-group-prepend">
                      <span class="input-group-text"><i class="fas fa-pencil-alt"></i></span>
              ...
              <div class="form-group row {{ $errors->has('apikey') ? ' text-red' : '' }}">
                <label for="apikey" class="col-sm-2 col-form-label">{{ gc_language_render('admin.api_connection.apikey') }}</label>
                <div class="col-sm-10 ">
                  <div class="input-group mb-3">
                    <div class="input-group-prepend">
                      <span class="input-group-text"><i class="fas fa-pencil-alt"></i></span>
              ...
              <div class="form-group row {{ $errors->has('expire') ? ' text-red' : '' }}">
                <label for="expire" class="col-sm-2 col-form-label">{{ gc_language_render('admin.api_connection.expire') }}</label>
                <div class="col-sm-10 ">
                  <div class="input-group mb-3">
                    <div class="input-group-prepend">
              ...
              <div class="form-group row {{ $errors->has('status') ? ' text-red' : '' }}">
                <label for="status" class="col-sm-2 col-form-label">{{ gc_language_render('admin.api_connection.status') }}</label>
                <div class="col-sm-10 ">
                  <div class="input-group mb-3">
          ...
          <div class="card-body p-0">   
            <div class="row">
                <div class="col-md-12 m-2">
                  <input class="switch-data-config" data-on-text="ON API"  data-off-text="OFF API" name="api_mode" type="checkbox"  {{ (gc_config_global('api_mode')?'checked':'') }}>
                  <br>&nbsp; {!! gc_language_render('admin.api_connection.api_connection_required_help') !!}
                </div>
            </div>
        </div>
        </div>
      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-header with-border">
            <h3 class="card-title">{!! $title ?? '' !!}  | <input class="switch-data-config" data-on-text="ON CONNECTION"  data-off-text="OFF CONNECTION" name="api_connection_required" type="checkbox"  {{ (gc_config_global('api_connection_required')?'checked':'') }}></h3>
          </div>
...
$(document).ready(function() {
    $('#refreshkey').click(function(){
        ...
        $.ajax({
            method: 'get',
            url: '{{ gc_route_admin('admin_api_connection.generate_key') }}',
...
$("input.switch-data-config").bootstrapSwitch();
  $('input.switch-data-config').on('switchChange.bootstrapSwitch', function (event, state) {
      ...
      $.ajax({
        type: 'POST',
        dataType:'json',
        url: "{{ gc_route_admin('admin_config_global.update') }}",
        data: {
          "_token": "{{ csrf_token() }}",
          "name": $(this).attr('name'),
          "value": valueSet
