...
@section('main')
        ...
        <div class="card card-primary card-outline card-outline-tabs">
          <div class="card-header p-0 border-bottom-0">
            <ul class="nav nav-tabs" id="custom-tabs-four-tab" role="tablist">
              <li class="nav-item">
                <a class="nav-link" href="{{ gc_route_admin('admin_plugin', ['code' => strtolower($code)]) }}" >{{ gc_language_render('admin.plugin.local') }}</a>
              </li>
              <li class="nav-item">
                <a class="nav-link active" href="#" >{{ gc_language_render('admin.plugin.online') }}</a>
              </li>
              <li class="nav-item">
              ...
              <table class="table table-hover text-nowrap table-bordered">
                <thead>
                  <tr>
                    ...
                    <th>{{ gc_language_render('admin.plugin.compatible') }}</th>
                    <th>{{ gc_language_render('admin.plugin.auth') }}</th>
                    <th>{{ gc_language_render('admin.plugin.price') }}</th>
                    <th>{{ gc_language_render('admin.plugin.rated') }}</th>
                    <th><i class="fa fa-download" aria-hidden="true"></i></th>
                    <th>{{ gc_language_render('admin.plugin.date') }}</th>
  ...
  @php
    ...
    {
      $pluginAction = '<span title="'.gc_language_render('admin.plugin.located').'" class="btn btn-flat btn-default"><i class="fa fa-check" aria-hidden="true"></i></span>';

    } elseif(!in_array(config('grakan.core'), $scVersion)) {
      $pluginAction = '';
    } else {
      if(($plugin['is_free'] || $plugin['price_final'] == 0)) {
        $pluginAction = '<span onClick="installPlugin($(this),\''.$plugin['key'].'\', \''.$plugin['file'].'\');" title="'.gc_language_render('admin.plugin.install').'" type="button" class="btn btn-flat btn-success"><i class="fa fa-plus-circle"></i></span>';
      } else {
        $pluginAction = '';
      }
    }
@endphp
                      <tr>
                        <td>{!! gc_image_render($plugin['image'],'50px', '', $plugin['name']) !!}</td>
                        <td>{{ $plugin['key'] }}</td>
                        ...
                        <td>
                          ...
                          <a href="{{ $plugin['link'] }}" title="Link home">
                            <span class="btn btn-flat btn-primary" type="button">
                              <i class="fa fa-chain-broken" aria-hidden="true"></i> {!! gc_language_render('admin.plugin.link') !!}
                            </span>
                          </a>
                        </td>  
                      </tr>
...
<script type="text/javascript">
  function installPlugin(obj,key, path) {
      ...
      $.ajax({
        type: 'POST',
        dataType:'json',
        url: '{{ gc_route_admin('admin_plugin_online.install') }}',
        data: {
          "_token": "{{ csrf_token() }}",
          "key":key,
          "path":path,
