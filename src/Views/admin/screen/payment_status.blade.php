    ...
    <div class="card">
      <div class="card-header">
        ...
        <div class="btn-group float-right" style="margin-right: 5px">
            <a href="{{ gc_route_admin('admin_payment_status.index') }}" class="btn  btn-flat btn-default" title="List"><i class="fa fa-list"></i><span class="hidden-xs"> {{ gc_language_render('admin.back_list') }}</span></a>
        </div>
      @endif
      </div>
      <!-- /.card-header -->
      ...
      <form action="{{ $url_action }}" method="post" accept-charset="UTF-8" class="form-horizontal" id="form-main">
        <div class="card-body">
            @includeIf($templatePathAdmin.'forms.input', ['col' => 10, 'name' => 'name', 'data' => $payment_status ?? null, 'label' => gc_language_render('admin.payment_status.name')])
        </div>
        <!-- /.card-body -->
        @csrf
            ...
            $.ajax({
                ...
                success: function (data) {
                    if(data.error == 1){
                      ...
                      window.location.replace('{{ gc_route_admin('admin_payment_status.index') }}');
                    }
