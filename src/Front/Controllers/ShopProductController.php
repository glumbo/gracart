...
class ShopProductController extends RootFrontController
{
    ...
    private function _allProducts()
    {
        ...
        return view(
            ...
            array(
                'title'       => gc_language_render('front.all_product'),
                'keyword'     => '',
                'description' => '',
                'products'    => $products,
                ...
                'breadcrumbs' => [
                    ['url'    => '', 'title' => gc_language_render('front.all_product')],
                ],
            )
        );
    }
    ...
    private function _productDetail($alias)
    {
        ...
        if ($product && $product->status && (!gc_config('product_stock', $storeId) || gc_config('product_display_out_of_stock', $storeId) || $product->stock > 0)) {
            //Update last view
            $product->view += 1;
            $product->date_lastview = gc_time_now();
            $product->save();
            //End last viewed

            //Product last view
            $arrlastView = empty(\Cookie::get('productsLastView')) ? array() : json_decode(\Cookie::get('productsLastView'), true);
            $arrlastView[$product->id] = gc_time_now();
            arsort($arrlastView);
            ...
            $productRelation = (new ShopProduct)
                ...
                ->setLimit(gc_config('product_relation', $storeId))
                ->setRandom()
                ->getData();

            gc_check_view($this->templatePath . '.screen.shop_product_detail');
            return view(
                ...
                array(
                    ...
                    'og_image'        => gc_file($product->getImage()),
                    'layout_page'     => 'shop_product_detail',
                    'breadcrumbs'     => [
                        ['url'        => gc_route('shop'), 'title' => gc_language_render('front.shop')],
                        $dataCategoryFirst,
