...
class ShopCartController extends RootFrontController
{
    ...
    private function _getCart()
    {
        ...
        gc_check_view($this->templatePath . '.screen.shop_cart');
        return view(
            $this->templatePath . '.screen.shop_cart',
            [
                'title'           => gc_language_render('cart.cart_title'),
                'description'     => '',
                'keyword'         => '',
                'cart'            => $cart,
                ...
                'breadcrumbs'  => [
                    ['url' => '', 'title' => gc_language_render('cart.cart_title')],
                ],
            ]
        );
    }
    ...
    public function prepareCheckout()
    {
        ...
        //Not allow for guest
        if (!gc_config('shop_allow_guest') && !$customer) {
            return redirect(gc_route('login'));
        }
        ...
        if (!$storeId) {
            return redirect(gc_route('cart'))->with(['error' => gc_language_render('cart.cart_store_id_notfound')]);
        }
        ...
        if (empty($cartGroup[$storeId])) {
            return redirect(gc_route('cart'))->with(['error' => gc_language_render('cart.cart_store_empty')]);
        }
        ...
        if (count($arrErrorQty)) {
            return redirect(gc_route('cart'))->with(['arrErrorQty' => $arrErrorQty, 'error'=> gc_language_render('cart.have_error')]);
        }
        //End check minimum
        ...
        return redirect(gc_route('checkout'));
    }
    ...
    private function _getCheckout()
    {
        ...
        if (!$dataCheckout || !$storeCheckout) {
            return redirect(gc_route('cart'))->with(['error' => gc_language_render('cart.cart_empty')]);
        }

        //Shipping
        $moduleShipping = gc_get_plugin_installed('shipping');
        $sourcesShipping = gc_get_all_plugin('shipping');
        $shippingMethod = array();
        foreach ($moduleShipping as $module) {
            if (array_key_exists($module['key'], $sourcesShipping)) {
                $moduleClass = gc_get_class_plugin_config('shipping', $module['key']);
                $shippingMethod[$module['key']] = (new $moduleClass)->getData();
            }
        }

        //Payment
        $modulePayment = gc_get_plugin_installed('payment');
        $sourcesPayment = gc_get_all_plugin('payment');
        $paymentMethod = array();
        ...
        //Total
        $moduleTotal = gc_get_plugin_installed('total');
        $sourcesTotal = gc_get_all_plugin('total');
        $totalMethod = array();
        ...
        //Process captcha
        $viewCaptcha = '';
        if (gc_captcha_method() && in_array('checkout', gc_captcha_page())) {
            if (view()->exists(gc_captcha_method()->pathPlugin.'::render')) {
                $dataView = [
                    'titleButton' => gc_language_render('cart.checkout'),
        ...
        //Check view
        gc_check_view($this->templatePath . '.screen.shop_checkout');

        return view(
            $this->templatePath . '.screen.shop_checkout',
            [
                'title'           => gc_language_render('cart.checkout'),
                'description'     => '',
                'keyword'         => '',
                ...
                'breadcrumbs'     => [
                    ['url'        => '', 'title' => gc_language_render('cart.checkout')],
                ],
            ]
        );
    }
    ...
    public function processCheckout()
    {
        ...
        $dataMap = gc_order_mapping_validate();
        ...
        if (count($arrErrorQty)) {
            return redirect(gc_route('cart'))->with('arrErrorQty', $arrErrorQty);
        }
        //End check minimum

        return redirect(gc_route('checkout.confirm'))->with('step', 'checkout.confirm');
    }
    ...
    private function _getCheckoutConfirm()
    {
        ...
        if (gc_config('shipping_off')) {
            ...
            $classShippingMethod = gc_get_class_plugin_config('Shipping', $shippingMethod);
        ...
        if (gc_config('payment_off')) {
            ...
            $classPaymentMethod = gc_get_class_plugin_config('Payment', $paymentMethod);
            $paymentMethodData = (new $classPaymentMethod)->getData();
        }

        //Check plugin invalid
        if (!gc_config(session('shippingMethod'))) {
            return redirect(gc_route('cart'))->with(['error' => 'Plugin shipping invalid!']);
        }
        if (!gc_config(session('paymentMethod'))) {
            return redirect(gc_route('cart'))->with(['error' => 'Plugin payment invalid!']);
        }
        ...
        gc_check_view($this->templatePath . '.screen.shop_checkout_confirm');
        return view(
            $this->templatePath . '.screen.shop_checkout_confirm',
            [
                'title'              => gc_language_render('checkout.page_title'),
                ...
                'breadcrumbs'        => [
                    ['url'           => '', 'title' => gc_language_render('checkout.page_title')],
                ],
            ]
        );
    }
    ...
    public function addOrder(Request $request)
    {
        ...
        $dataOrder['currency']        = gc_currency_code();
        $dataOrder['exchange_rate']   = gc_currency_rate();
        ...
        $dataOrder['created_at']      = gc_time_now();
        ...
        foreach ($dataCheckout as $cartItem) {
            ...
            $arrDetail['price']       = gc_currency_value($cartItem->price);
            ...
            $arrDetail['total_price'] = gc_currency_value($cartItem->price) * $cartItem->qty;
        ...
        if ($newOrder['error'] == 1) {
            gc_report($newOrder['msg']);
            return redirect(gc_route('cart'))->with(['error' => $newOrder['msg']]);
        }
        //Set session orderID
        ...
        if ($address_process == 'new') {
            ...
            //Process escape
            $addressNew = gc_clean($addressNew);
        ...
        $paymentMethod = gc_get_class_plugin_controller('Payment', session('paymentMethod'));
    ...
    public function addToCart()
    {
        ...
        if (!$product) {
            return response()->json(
                [
                    'error' => 1,
                    'msg' => gc_language_render('front.data_notfound'),
                ]
            );
        }
        ...
        if ($product->allowSale()) {
            ...
            return redirect(gc_route('cart'))
                ->with(
                    ['success' => gc_language_render('cart.add_to_cart_success', ['instance' => 'cart'])]
                );
        } else {
            return redirect(gc_route('cart'))
                ->with(
                    ['error' => gc_language_render('product.dont_allow_sale', ['sku' => $product->sku])]
                );
        }
    }
    ...
    public function addToCartAjax(Request $request)
    {
        ...
        switch ($instance) {
            case 'default':
                if ($product->attributes->count() || $product->kind == GC_PRODUCT_GROUP) {
                ...
                if ($product->allowSale()) {
                    ...
                    return response()->json(
                        [
                            'error' => 1,
                            'msg' => gc_language_render('product.dont_allow_sale', ['sku' => $product->sku]),
                        ]
                    );
                }
                break;
            ...
            default:
                ...
                if (!in_array($id, ${'arrID' . $instance})) {
                    ...
                    return response()->json(
                        [
                            'error' => 1,
                            'msg' => gc_language_render('cart.item_exist_in_cart', ['instance' => $instance]),
                        ]
                    );
                }
                break;
        ...
        return response()->json(
            [
                ...
                'msg'        => gc_language_render('cart.add_to_cart_success', ['instance' => ($instance == 'default') ? 'cart' : $instance]),
            ]
        );
    }
    ...
    public function updateToCart(Request $request)
    {
        ...
        if ($product->stock < $new_qty && !gc_config('product_buy_out_of_stock', $product->store_id)) {
            return response()->json(
                [
                    'error' => 1,
                    'msg' => gc_language_render('cart.item_over_qty', ['sku' => $product->sku, 'qty' => $new_qty]),
                ]
            );
        } else {
    ...
    private function _wishlist()
    {
        ...
        gc_check_view($this->templatePath . '.screen.shop_wishlist');
        return view(
            $this->templatePath . '.screen.shop_wishlist',
            array(
                'title'       => gc_language_render('cart.page_wishlist_title'),
                'description' => '',
                'keyword'     => '',
                'wishlist'    => $wishlist,
                'layout_page' => 'shop_wishlist',
                'breadcrumbs' => [
                    ['url'    => '', 'title' => gc_language_render('cart.page_wishlist_title')],
                ],
            )
        );
    }
    ...
    private function _compare()
    {
        ...
        gc_check_view($this->templatePath . '.screen.shop_compare');
        return view(
            $this->templatePath . '.screen.shop_compare',
            array(
                'title'       => gc_language_render('cart.page_compare_title'),
                'description' => '',
                'keyword'     => '',
                'compare'     => $compare,
                'layout_page' => 'shop_compare',
                'breadcrumbs' => [
                    ['url'    => '', 'title' => gc_language_render('cart.page_compare_title')],
                ],
            )
        );
    }
    ...
    private function _clearCart($instance = 'cart')
    {
        ...
        return redirect(gc_route($instance));
    }
    ...
    public function completeOrder()
    {
        ...
        $classPaymentConfig = gc_get_class_plugin_config('Payment', $paymentMethod);
        if (method_exists($classPaymentConfig, 'endApp')) {
            (new $classPaymentConfig)->endApp();
        }

        $classShippingConfig = gc_get_class_plugin_config('Shipping', $shippingMethod);
        ...
        if ($totalMethod && is_array($totalMethod)) {
            foreach ($totalMethod as $keyMethod => $valueMethod) {
                $classTotalConfig = gc_get_class_plugin_config('Total', $keyMethod);
        ...
        gc_event_order_success($order = ShopOrder::find($orderID));

        // Process after order compled: send mail, data response ...
        $dataResponse = $this->processAfterOrderSuccess($orderID);

        return redirect(gc_route('order.success'))->with($dataResponse);
    }
    ...
    private function _orderSuccess()
    {
        ...
        gc_check_view($this->templatePath . '.screen.shop_order_success');
        $orderInfo = ShopOrder::with('details')->find(session('orderID'))->toArray();
        return view(
            $this->templatePath . '.screen.shop_order_success',
            [
                'title'       => gc_language_render('checkout.success_title'),
                'orderInfo'   => $orderInfo,
                'layout_page' => 'shop_order_success',
                'breadcrumbs' => [
                    ['url'    => '', 'title' => gc_language_render('checkout.success_title')],
                ],
            ]
        );
    }
    ...
    private function processAfterOrderSuccess (string $orderID)
    {
        //Clear session
        $this->clearSession();
        return gc_order_process_after_success($orderID);
    }
}
