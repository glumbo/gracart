...
if (file_exists(app_path('Http/Controllers/ShopContentController.php'))) {
    ...
    $nameSpaceFrontContent = 'Glumbo\Gracart\Front\Controllers';
}

if (file_exists(app_path('Http/Controllers/ShopStoreController.php'))) {
    $nameSpaceFrontStore = 'App\Http\Controllers';
} else {
    $nameSpaceFrontStore = 'Glumbo\Gracart\Front\Controllers';
}
...
Route::group(
    [
        'middleware' => GC_FRONT_MIDDLEWARE,
    ],
...
if (config('grakan.ecommerce_mode', 1)) {
    Route::middleware(GC_FRONT_MIDDLEWARE)
    ...
    Route::group(
        ...
        function () use ($langUrl, $nameSpaceFrontStore) {
            $prefixShop = gc_config('PREFIX_SHOP') ?? 'shop';
...
Route::group(
    ...
    function () use ($suffix, $nameSpaceFrontContent, $nameSpaceFrontStore) {
        $prefixSearch = gc_config('PREFIX_SEARCH')??'search';
        $prefixContact = gc_config('PREFIX_CONTACT')??'contact';
        $prefixAbout = gc_config('PREFIX_ABOUT')??'about';
        $prefixNews = gc_config('PREFIX_NEWS')??'news';
...
Route::group(
    ...
    function () use ($nameSpaceFrontContent) {
        ...
        Route::get('locale/{code}', function ($code) {
            ...
            if (gc_route('home.lang') === redirect()->back()->getTargetUrl()
            ) {
                return redirect(gc_route('home.lang', ['lang' => $code]));
            }
