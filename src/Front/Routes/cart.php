...
Route::group(
    ...
    function ($router) use ($suffix, $nameSpaceFrontCart) {
        $prefixCartWishlist = gc_config('PREFIX_CART_WISHLIST') ?? 'wishlist';
        $prefixCartCompare = gc_config('PREFIX_CART_COMPARE') ?? 'compare';
        $prefixCartDefault = gc_config('PREFIX_CART_DEFAULT') ?? 'cart';
