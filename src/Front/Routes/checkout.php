...
if (file_exists(app_path('Http/Controllers/ShopCartController.php'))) {
    ...
    $nameSpaceFrontCart = 'Glumbo\Gracart\Front\Controllers';
}
Route::group(
    [
        'prefix' => $langUrl
    ...
    function ($router) use ($suffix, $nameSpaceFrontCart) {
        $prefixCartCheckout = gc_config('PREFIX_CART_CHECKOUT') ?? 'checkout';
        $prefixCartCheckoutConfirm = gc_config('PREFIX_CART_CHECKOUT_CONFIRM') ?? 'checkout-confirm';
        $prefixOrderSuccess = gc_config('PREFIX_ORDER_SUCCESS') ?? 'order-success';
