...
class ShopOrderTotal extends Model
{
    ...
    protected $table = GC_DB_PREFIX.'shop_order_total';
    ...
    public static function processDataTotal(array $objects = [])
    {
        ...
        //Set subtotal
        $arraySubtotal = [
            'title' => gc_language_render('order.totals.sub_total'),
            'code' => 'subtotal',
            'value' => $subtotal,
            'text' => gc_currency_render_symbol($subtotal),
            'sort' => self::POSITION_SUBTOTAL,
        ];

        //Set tax
        $arrayTax = [
            'title' => gc_language_render('order.totals.tax'),
            'code' => 'tax',
            'value' => $tax,
            'text' => gc_currency_render_symbol($tax),
        ...
        $arrayTotal = array(
            'title' => gc_language_render('order.totals.total'),
            'code' => 'total',
            'value' => $total,
            'text' => gc_currency_render_symbol($total),
    ...
    public static function getShippingMethod()
    {
        ...
        if ($shippingMethod) {
            $moduleClass = gc_get_class_plugin_config('Shipping', $shippingMethod);
            $returnModuleShipping = (new $moduleClass)->getData();
            $arrShipping = [
                'title' => $returnModuleShipping['title'],
                'code' => 'shipping',
                'value' => gc_currency_value($returnModuleShipping['value']),
                'text' => gc_currency_render($returnModuleShipping['value']),
    ...
    public static function getPaymentMethod()
    {
        ...
        if ($paymentMethod) {
            $moduleClass = gc_get_class_plugin_config('Paypal', $paymentMethod);
    ...
    public static function getTotal()
    {
        ...
        if ($totalMethod && is_array($totalMethod)) {
            foreach ($totalMethod as $keyMethod => $valueMethod) {
                ...
                $totalMethod[] = [
                    ...
                    'text' => gc_currency_render_symbol($returnModuleTotal['value']),
        ...
        if (!count($totalMethod)) {
            $totalMethod[] = array(
                'title' => gc_language_render('order.totals.discount'),
                'code' => 'discount',
                'value' => 0,
                'text' => 0,
                'sort' => self::POSITION_TOTAL_METHOD,
    ...
    {
        return array(
            'title' => gc_language_render('order.totals.received'),
            'code' => 'received',
            'value' => 0,
            'text' => 0,
    ...
    public static function getOtherFee()
    {
        ...
        if (!count($otherFeeMethod)) {
            $otherFeeMethod[] = array(
                'title' => gc_language_render('order.totals.other_fee'),
                'code' => 'other_fee',
                'value' => 0,
                'text' => 0,
                'sort' => self::POSITION_OTHER_FEE,
    ...
    protected static function boot()
    {
        ...
        static::creating(function ($model) {
            if (empty($model->{$model->getKeyName()})) {
                $model->{$model->getKeyName()} = gc_generate_id($type = 'shop_order_total');
            }
        });
    }
}
