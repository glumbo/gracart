...
class ShopSupplier extends Model
{
    ...
    public $table = GC_DB_PREFIX.'shop_supplier';
    protected $guarded = [];
    ...
    {
        return gc_route('supplier.detail', ['alias' => $this->alias, 'lang' => $lang ?? app()->getLocale()]);
    }
    ...
    protected static function boot()
    {
        ...
        static::deleting(
            function ($supplier) {
                ...
                ->where(GC_DB_PREFIX.'shop_custom_field_detail.rel_id', $supplier->id)
                ->where(GC_DB_PREFIX.'shop_custom_field.type', 'shop_supplier')
                ->delete();
            }
        );
        //Uuid
        static::creating(function ($model) {
            if (empty($model->{$model->getKeyName()})) {
                $model->{$model->getKeyName()} = gc_generate_id($type = 'shop_supplier');
            }
        });
    }
}
