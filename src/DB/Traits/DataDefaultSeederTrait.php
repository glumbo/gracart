...
namespace Glumbo\Gracart\DB\Traits;
...
trait DataDefaultSeederTrait
{
    ...
    public function runProcess()
    {
        ...
        $db = DB::connection(GC_CONNECTION);
        ...
        $db->table(GC_DB_PREFIX.'admin_menu')->insertOrIgnore($dataMenu);
        
        $dataAdminPermission = $this->dataAdminPermission(GC_ADMIN_PREFIX);
        $db->table(GC_DB_PREFIX.'admin_permission')->insertOrIgnore($dataAdminPermission);
        
        $dataAdminRole = $this->dataAdminRole();
        $db->table(GC_DB_PREFIX.'admin_role')->insertOrIgnore($dataAdminRole);
        
        $dataAdminTemplate = $this->dataAdminTemplate();
        $db->table(GC_DB_PREFIX.'admin_template')->insertOrIgnore($dataAdminTemplate);

        $dataAdminRolePermission = $this->dataAdminRolePermission();
        $db->table(GC_DB_PREFIX.'admin_role_permission')->insertOrIgnore($dataAdminRolePermission);

        $dataAdminRoleUser = $this->dataAdminRoleUser();
        $db->table(GC_DB_PREFIX.'admin_role_user')->insertOrIgnore($dataAdminRoleUser);

        $dataAdminUser = $this->dataAdminUser($this->adminUser, $this->adminPassword, $this->adminEmail);
        $db->table(GC_DB_PREFIX.'admin_user')->insertOrIgnore($dataAdminUser);

        $dataAdminConfig = $this->dataAdminConfig();
        $db->table(GC_DB_PREFIX.'admin_config')->insertOrIgnore($dataAdminConfig);

        $dataAdminStore = $this->dataAdminStore($this->adminEmail, $this->language_default, str_replace(['http://','https://', '/install.php'], '', url('/')));
        $db->table(GC_DB_PREFIX.'admin_store')->insertOrIgnore($dataAdminStore);

        $dataAdminStoreDescription = $this->dataAdminStoreDescription($this->title_en, $this->title_vi);
        $db->table(GC_DB_PREFIX.'admin_store_description')->insertOrIgnore($dataAdminStoreDescription);

        $dataShopLang = $this->dataShopLang();
        $db->table(GC_DB_PREFIX.'shop_language')->insertOrIgnore($dataShopLang);

        $dataPaymentStatus = $this->dataPaymentStatus();
        $db->table(GC_DB_PREFIX.'shop_payment_status')->insertOrIgnore($dataPaymentStatus);

        $dataShippingStatus = $this->dataShippingStatus();
        $db->table(GC_DB_PREFIX.'shop_shipping_status')->insertOrIgnore($dataShippingStatus);

        $dataShopLayoutPage = $this->dataShopLayoutPage();
        $db->table(GC_DB_PREFIX.'shop_layout_page')->insertOrIgnore($dataShopLayoutPage);

        $dataShopLayoutPosition = $this->dataShopLayoutPosition();
        $db->table(GC_DB_PREFIX.'shop_layout_position')->insertOrIgnore($dataShopLayoutPosition);

        $dataShopLink = $this->dataShopLink();
        $db->table(GC_DB_PREFIX.'shop_link')->insertOrIgnore($dataShopLink);

        $dataShopLinkStore = $this->dataShopLinkStore($dataShopLink);
        $db->table(GC_DB_PREFIX.'shop_link_store')->insertOrIgnore($dataShopLinkStore);

        $dataShippingStandard = $this->dataShippingStandard();
        $db->table(GC_DB_PREFIX.'shop_shipping_standard')->insertOrIgnore($dataShippingStandard);

        $dataAtrributeGroup = $this->dataAtrributeGroup();
        $db->table(GC_DB_PREFIX.'shop_attribute_group')->insertOrIgnore($dataAtrributeGroup);

        $dataCurrency = $this->dataCurrency();
        $db->table(GC_DB_PREFIX.'shop_currency')->insertOrIgnore($dataCurrency);

        $dataOrderStatus = $this->dataOrderStatus();
        $db->table(GC_DB_PREFIX.'shop_order_status')->insertOrIgnore($dataOrderStatus);

        $dataPage = $this->dataPage();
        $db->table(GC_DB_PREFIX.'shop_page')->insertOrIgnore($dataPage);

        $dataPageStore = $this->dataPageStore($dataPage);
        $db->table(GC_DB_PREFIX.'shop_page_store')->insertOrIgnore($dataPageStore);

        $dataPageDescription = $this->dataPageDescription($dataPage);
        $db->table(GC_DB_PREFIX.'shop_page_description')->insertOrIgnore($dataPageDescription);

        $dataApiConnection = $this->dataApiConnection();
        $db->table(GC_DB_PREFIX.'api_connection')->insertOrIgnore($dataApiConnection);

        $dataTax = $this->dataTax();
        $db->table(GC_DB_PREFIX.'shop_tax')->insertOrIgnore($dataTax);

        $dataWeight = $this->dataWeight();
        $db->table(GC_DB_PREFIX.'shop_weight')->insertOrIgnore($dataWeight);

        $dataLenght = $this->dataLenght();
        $db->table(GC_DB_PREFIX.'shop_length')->insertOrIgnore($dataLenght);

        $dataBannerType = $this->dataBannerType();
        $db->table(GC_DB_PREFIX.'shop_banner_type')->insertOrIgnore($dataBannerType);

        $dataProductProperty = $this->dataProductProperty();
        $db->table(GC_DB_PREFIX.'shop_product_property')->insertOrIgnore($dataProductProperty);

        $dataLinkGroup = $this->dataLinkGroup();
        $db->table(GC_DB_PREFIX.'shop_link_group')->insertOrIgnore($dataLinkGroup);
    ...
    public function dataMenu() {
        $dataMenu = [
            ...
            ['id' => 42,'parent_id' => 35,'sort' => 100,'title' => 'grakan::admin.menu_titles.plugin_other','icon' => 'far fa-circle','uri' => 'admin::plugin/other','key' => null,'type' => 0],
            ['id' => 43,'parent_id' => 35,'sort' => 4,'title' => 'grakan::admin.menu_titles.plugin_cms','icon' => 'fab fa-modx','uri' => 'admin::plugin/cms','key' => null,'type' => 0],
        ...
        // If use ecommerce
        if (config('grakan.ecommerce_mode', 1)) {
    ...
    public function dataMenuShop() {
        $dataMenu = [
            ...
            ['id' => 39,'parent_id' => 35,'sort' => 0,'title' => 'grakan::admin.menu_titles.plugin_payment','icon' => 'far fa-money-bill-alt','uri' => 'admin::plugin/payment','key' => null,'type' => 0],
            ['id' => 40,'parent_id' => 35,'sort' => 1,'title' => 'grakan::admin.menu_titles.plugin_shipping','icon' => 'fas fa-ambulance','uri' => 'admin::plugin/shipping','key' => null,'type' => 0],
            ['id' => 41,'parent_id' => 35,'sort' => 2,'title' => 'grakan::admin.menu_titles.plugin_total','icon' => 'fas fa-atom','uri' => 'admin::plugin/total','key' => null,'type' => 0],
            ['id' => 53,'parent_id' => 35,'sort' => 3,'title' => 'grakan::admin.menu_titles.plugin_fee','icon' => 'fas fa-box','uri' => 'admin::plugin/fee','key' => null,'type' => 0],
    ...
    public function dataAdminTemplate() {
        $dataAdminTemplate = [
            ['id' => '1','key' => 'grakan-light','name' => 'S-Cart Light','status' => 1]
        ];
    ...
    public function dataAdminStore($email, $language, $domain) {
        $dataAdminStore = [
            ['id' => 1,'logo' => 'data/logo/scart-mid.png','template' => 'grakan-light','phone' => '0123456789','long_phone' => 'Support: 0987654321','email' => $email,'time_active' =>'','address' => '123st - abc - xyz','language' => $language,'currency' => 'USD','code' => 'grakan','domain' => $domain]
        ];
    ...
    public function dataAdminStoreDescription($titleE, $titleV) {
        $dataAdminStoreDescription = [
            ['store_id' => GC_ID_ROOT,'lang' => 'en','title' => $titleE,'description' => 'Free website shopping cart for business','keyword' => '','maintain_content' => '<center><img src="/images/maintenance.png" />
            <h3><span style="color:#e74c3c;"><strong>Sorry! We are currently doing site maintenance!</strong></span></h3>
            </center>','maintain_note' => 'Website is in maintenance mode!'],
            ['store_id' => GC_ID_ROOT,'lang' => 'vi','title' => $titleV,'description' => 'Laravel shopping cart for business','keyword' => '','maintain_content' => '<center><img src="/images/maintenance.png" />
    ...
    public function dataShopLink() {
        $dataShopLink = [
            ...
            ['id' => 3,'name' => 'About us','url' => 'https://grakan.org/en/about.html','target' => '_self','module' => null,'group' => 'menu', 'type' => null, 'collection_id' => 2, 'status' => '1','sort' => '10'],
            ['id' => 4,'name' => 'Github','url' => 'https://github.com/grakan/grakan','target' => '_self','module' => null,'group' => 'menu', 'type' => null, 'collection_id' => 2, 'status' => '1','sort' => '20'],
    ...
    public function dataShopLinkStore($dataShopLink) {
        foreach ($dataShopLink as $key => $row) {
            $dataShopLinkStore[] = ['link_id' => $row['id'],'store_id' => GC_ID_ROOT];
        }
    ...
    public function dataPageStore($dataPage) {
        ...
        foreach ($dataPage as $key => $row) {
            $dataPageStore[] = ['page_id' => $row['id'],'store_id' => GC_ID_ROOT];
        }
    ...
    public function updateDataVersion() {
        ...
        $db = DB::connection(GC_CONNECTION);
        $schema = Schema::connection(GC_CONNECTION);
        //Ony use updated v8.0 -> v8.1
        //--Update menu admin
        $db->table(GC_DB_PREFIX.'admin_menu')->where('id','27')->update(['parent_id' => 65]);
        $db->table(GC_DB_PREFIX.'admin_menu')->whereIn('id',['17','18'])->update(['parent_id' => 2]);
        $db->table(GC_DB_PREFIX.'admin_menu')->whereIn('id',['36','50'])->update(['parent_id' => 70]);
        
        // Add column
        if (!$schema->hasColumn(GC_DB_PREFIX.'admin_store', 'og_image')) {
            $schema->table(GC_DB_PREFIX.'admin_store',
        ...
        //Notice
        if (!$schema->hasTable(GC_DB_PREFIX.'admin_notice')) {
            $schema->create(GC_DB_PREFIX . 'admin_notice', function (Blueprint $table) {
