...
trait DataSampleCmsSeederTrait
{
    ...
    public function runProcess()
    {
        ...
        $db->table(GC_DB_PREFIX.'shop_news')->insert($dataNews);

        $dataNewsDescription = $this->dataNewsDescription($arrayIdNews);
        $db->table(GC_DB_PREFIX.'shop_news_description')->insert($dataNewsDescription);

        $dataNewsStore = $this->dataNewsStore($arrayIdNews);
        $db->table(GC_DB_PREFIX.'shop_news_store')->insert($dataNewsStore);

        $dataBanner = $this->dataBanner();
        $db->table(GC_DB_PREFIX.'shop_banner')->insert($dataBanner);

        $dataBannerStore = $this->dataBannerStore($dataBanner);
        $db->table(GC_DB_PREFIX.'shop_banner_store')->insert($dataBannerStore);
    }
    ...
    public function dataNewsStore($ids) {
        ...
        foreach ($ids as $key => $id) {
            $dataNewsStore[] = [
                'news_id' => $id, 'store_id' => GC_ID_ROOT
            ];
        }
    ...
    public function dataBanner() {
        $dataBanner = [];
        $dataBanner[] = ['id' => (string)Str::orderedUuid(), 'title' => 'Banner home 1 (grakan-light)','image' => '/data/banner/banner-home-1.jpg','status' => 1, 'html' => '<h1 class="swiper-title-1" data-caption-animate="fadeScale" data-caption-delay="100">Top-notch Furniture</h1><p class="biggest text-white-70" data-caption-animate="fadeScale" data-caption-delay="200">Sofa Store provides the best furniture and accessories for homes and offices.</p><div class="button-wrap" data-caption-animate="fadeInUp" data-caption-delay="300"> <span class="button button-zachem-tak-delat button-white button-zakaria"> Shop now</span> </div>', 'target' => '_self', 'type' => 'banner'];
        $dataBanner[] = ['id' => (string)Str::orderedUuid(), 'title' => 'Banner home 2 (grakan-light)','image' => '/data/banner/banner-home-2.jpg','status' => 1, 'html' => '<h1 class="swiper-title-1" data-caption-animate="fadeScale" data-caption-delay="100">Top-notch Furniture</h1><p class="biggest text-white-70" data-caption-animate="fadeScale" data-caption-delay="200">Sofa Store provides the best furniture and accessories for homes and offices.</p><div class="button-wrap" data-caption-animate="fadeInUp" data-caption-delay="300"> <span class="button button-zachem-tak-delat button-white button-zakaria"> Shop now</span> </div>', 'target' => '_self', 'type' => 'banner'];
        $dataBanner[] = ['id' => (string)Str::orderedUuid(), 'title' => 'Banner breadcrumb (grakan-light)','image' => '/data/banner/breadcrumb.jpg','status' => 1, 'html' => '', 'target' => '_self', 'type' => 'breadcrumb'];
        $dataBanner[] = ['id' => (string)Str::orderedUuid(), 'title' => 'Banner store (grakan-light)','image' => '/data/banner/banner-store.jpg','status' => 1, 'html' => '', 'target' => '_self', 'type' => 'banner-store'];
        return $dataBanner;
    }
    ...
    public function dataBannerStore($dataBanner) {
        ...
        foreach ($dataBanner as  $banner) {
            $dataBannerStore[] = [
                'banner_id' => $banner['id'], 'store_id' => GC_ID_ROOT
            ];
        }
