...
trait DataSampleShopSeederTrait
{
    ...
    public function runProcess()
    {
        ...
        $db->table(GC_DB_PREFIX.'shop_category')->insert($dataCategory);

        $dataCategoryStore = $this->dataCategoryStore($dataCategory);
        $db->table(GC_DB_PREFIX.'shop_category_store')->insert($dataCategoryStore);

        $dataCategoryDescription = $this->dataCategoryDescription($dataCategory);
        $db->table(GC_DB_PREFIX.'shop_category_description')->insert($dataCategoryDescription);

        $dataSupplier = $this->dataSupplier();
        $db->table(GC_DB_PREFIX.'shop_supplier')->insert($dataSupplier);

        $dataBrand = $this->dataBrand();
        $db->table(GC_DB_PREFIX.'shop_brand')->insert($dataBrand);

        $dataBrandStore = $this->dataBrandStore($dataBrand);
        $db->table(GC_DB_PREFIX.'shop_brand_store')->insert($dataBrandStore);
        ...
        $db->table(GC_DB_PREFIX.'shop_product')->insert($dataProduct);

        $dataProductStore = $this->dataProductStore($arrayIdProduct);
        $db->table(GC_DB_PREFIX.'shop_product_store')->insert($dataProductStore);

        $dataProductDescription = $this->dataProductDescription($dataProduct);
        $db->table(GC_DB_PREFIX.'shop_product_description')->insert($dataProductDescription);

        $db->table(GC_DB_PREFIX.'shop_product_build')->insert($mappingIdProduct['arrBuild']);
        $db->table(GC_DB_PREFIX.'shop_product_group')->insert($mappingIdProduct['arrGroup']);
        $db->table(GC_DB_PREFIX.'shop_product_promotion')->insert($mappingIdProduct['arrPromotion']);
        $db->table(GC_DB_PREFIX.'shop_product_image')->insert($mappingIdProduct['arrImage']);
        $db->table(GC_DB_PREFIX.'shop_product_category')->insert($mappingIdProduct['arrCategory']);
        $db->table(GC_DB_PREFIX.'shop_product_attribute')->insert($mappingIdProduct['arrAtt']);
    }
    ...
    public function dataCategoryStore($dataCate) {
        ...
        foreach ($dataCate as  $cate) {
            $dataCategoryStore[] = [
                'category_id' => $cate['id'], 'store_id' => GC_ID_ROOT
            ];
        }
    ...
    public function dataSupplier() {
        ...
        $dataSupplier[$id1] = ['id' => $id1, 'alias' => 'supplier-abc', 'name' => 'Supplier ABC','email' => 'abc@gmail.com','phone' => '0123456789', 'image' => '/data/supplier/supplier.jpg', 'store_id' => GC_ID_ROOT];
        $dataSupplier[$id2] = ['id' => $id2, 'alias' => 'supplier-xyz', 'name' => 'Supplier XYZ','email' => 'xyz@gmail.com','phone' => '0987654321', 'image' => '/data/supplier/supplier.jpg', 'store_id' => GC_ID_ROOT];
        return $dataSupplier;
    }
    ...
    public function dataBrandStore($dataBrand) {
        ...
        foreach ($dataBrand as  $brand) {
            $dataBrandStore[] = [
                'brand_id' => $brand['id'], 'store_id' => GC_ID_ROOT
            ];
        }
    ...
    public function dataProductStore($arrayIdProduct) {
        ...
        foreach ($arrayIdProduct as  $id) {
            $dataProductStore[] = [
                'product_id' => $id, 'store_id' => GC_ID_ROOT
            ];
        }
