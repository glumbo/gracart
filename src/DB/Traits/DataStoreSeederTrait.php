...
trait DataStoreSeederTrait
{
    public function getTemplateDefault() {
        return  empty(session('lastStoreTemplate')) ? 'grakan-light' : session('lastStoreTemplate');
    }
    /**
    ...
    {
        $storeId = empty(session('lastStoreId')) ? GC_ID_ROOT : session('lastStoreId');

        $db = DB::connection(GC_CONNECTION);

        $dataConfig = $this->dataConfig($storeId);
        $db->table(GC_DB_PREFIX.'admin_config')->insert($dataConfig);

        $dataEmailTemplate = $this->dataEmailTemplate($storeId);
        $db->table(GC_DB_PREFIX.'shop_email_template')->insert($dataEmailTemplate);
        
        $dataShopLink = $this->dataShopLink();
        $db->table(GC_DB_PREFIX.'shop_link')->insert($dataShopLink);

        $dataShopLinkStore = $this->dataShopLinkStore($dataShopLink, $storeId);
        $db->table(GC_DB_PREFIX.'shop_link_store')->insert($dataShopLinkStore);


        if (file_exists($fileProcess = resource_path() . '/views/templates/'.$this->getTemplateDefault().'/Provider.php')) {
            include_once $fileProcess;
            if (function_exists('gc_template_install_store')) {
    ...
    public function dataConfig($storeId) {
        $dataConfig = [
            ...
            ['group' => '','code' => 'admin_custom_config','key' => 'facebook_url','value' => 'https://www.facebook.com/GCart.Ecommerce/','sort' => '0','detail' => 'admin.admin_custom_config.facebook_url','store_id' => $storeId],
